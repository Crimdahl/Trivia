...
class Settings(object):
    ...
    def Save(self, settings_file):
        try:
            ...
            Log("Settings Save: Failed to save settings to the file: " + str(e), LoggingLevel.str_to_int.get("Fatal"))
            raise e
        return
        ...
        if str(data.Message).startswith("!trivia") and (Parent.HasPermission(data.User, script_settings.permissions_admins, "") or user_id == "216768170") and data.GetParamCount() > 0:
            ...
            if subcommand == "stop":
                ...
                if data.GetParamCount() == 2 and len(current_questions_list) > 0:
                    ...
                    Log("Trivia Load: Subcommand used, but no questions exist that can be loaded.", LoggingLevel.str_to_int.get("Info"))
                ...
                if data.GetParamCount() == 2:
                    ...
                    try:
                        ...
                        Log("Trivia Add: No point value was supplied. Using the default point value.", LoggingLevel.str_to_int.get("Debug"))
                    ...
                    try:
                        ...
                        if not current_game == "":
                            ...
                            Log("Trivia Add: Question not added. No game attribute detected in command call.", LoggingLevel.str_to_int.get("Warn"))
                    ...
                    try:
                        ...
                        Log("Trivia Add: Question not added. No question attribute detected in command call.", LoggingLevel.str_to_int.get("Warn"))
                    ...
                    try:
                        ...
                        #If the attribute is not found, display an error
                        Log("Trivia Add: Question not added. No answers attribute detected in command call.", LoggingLevel.str_to_int.get("Warn"))
                ...
                if data.GetParamCount() == 2:
                    ...
                    try:
                        ...
                        if script_settings.enable_game_detection:
                            ...
                            #Log("Number of indices after the question that was removed: " + str(len(question_index_map[question_index:])) + ".", LoggingLevel.str_to_int.get("Debug"))
                            question_index_map[question_index:] = [index - 1 for index in question_index_map[question_index:]]
                        else:
                            #Log("Trivia Remove: A question is being removed without Enable Game Detection Mode. Index " + str(question_index) + ".", LoggingLevel.str_to_int.get("Debug"))
                            master_questions_list.pop(question_index)
                        if question_index == current_question_index:
                            current_question_index = -1
                        if SaveTrivia():
                            Log("Trivia Remove: A question has been removed: " + str(old_question), LoggingLevel.str_to_int.get("Info"))
                            Post("Question removed.")
                    except (ValueError, IndexError) as e:
                        Log("Trivia Remove: Question could not be removed: " + str(e), LoggingLevel.str_to_int.get("Warn"))
                ...
                if data.GetParamCount() == 2:
                    ...
                    try:
                        ...
                        Log("Trivia Modify: Trivia Modify subcommand supplied a non-integer question index.", LoggingLevel.str_to_int.get("Warn"))
                    ...
                    if newGame: 
                        ...
                        if script_settings.enable_game_detection:
                            Log("Trivia Modify: A question's game is being modified with Enable Game Detection Mode. Master Question List Index " + str(question_index_map[question_index]) + 
                                    ". Current Question List Index " + str(question_index) + ".", LoggingLevel.str_to_int.get("Debug"))
                    ...
                    if newPoints:
                        try:
                            ...
                            Log("Trivia Modify: Trivia Modify subcommand supplied a non-integer point value.", LoggingLevel.str_to_int.get("Warn"))
            ...
            Log(data.UserName + " attempted to use trivia admin commands without permission. " + str(data.Message), LoggingLevel.str_to_int.get("Info"))
...
def Tick():
    ...
    if active and (not script_settings.run_only_when_live or (script_settings.run_only_when_live and Parent.IsLive())):
        ...
        #Log("Current time: " + str(current_time) + ". Update time: " + str(next_question_file_update_time))
            ...
            #Log("Tick: Updating Question File. Current time: " + str(current_time) + " vs " + str(next_question_file_update_time), LoggingLevel.str_to_int.get("Debug"))
            #UpdateCurrentQuestionFile()
    return
...
def GetAttribute(attribute, message):
    Log("GetAttribute: Called with message \"" + message + "\" looking for attribute \"" + attribute + "\".", LoggingLevel.str_to_int.get("Debug"))
    ...
    try:
        ...
        Log("GetAttribute: Attribute found at index " + str(index_of_beginning_of_attribute), LoggingLevel.str_to_int.get("Debug"))
    except ValueError as e:
    ...
    #Log("GetAttribute: " + attribute + " successfully retrieved with a value of \"" + result + "\".", LoggingLevel.str_to_int.get("Debug"))
    return result
...
def SaveTrivia():
    try:        
        ...
        if not os.path.exists(questions_file):
            ...
            Log("SaveTrivia: The trivia file was not found. A new one was created.", LoggingLevel.str_to_int.get("Warn"))
...
def LoadTrivia():
    ...
    Log("LoadTrivia: Questions loaded into master list: " + str(len(master_questions_list)) + ". Questions currently being used: " + str(len(current_questions_list)), LoggingLevel.str_to_int.get("Info"))
...
def ReloadSettings(jsonData):
    ...
    if not previous_game_detection == script_settings.enable_game_detection:
        ...
        if not script_settings.enable_game_detection:
            ...
            current_game = json.loads(Parent.GetRequest(twitch_api_source + script_settings.twitch_channel_name, {})).get("response")
            Log("ReloadSettings: Game Detection activated. Most recent game identified as " + str(current_game) + ". Reloading questions.", LoggingLevel.str_to_int.get("Info"))
...
def Log(message, level = LoggingLevel.str_to_int.get("All")):
    if script_settings.enable_file_logging:
        ...
        file.writelines(str(datetime.now()).ljust(26) + " " + str(LoggingLevel.int_to_string.get(level) + ":").ljust(10) + message + "\n")
        file.close()
